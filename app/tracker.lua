-- ===================================================================
-- 🔥 YEAGER NEXUS HUB | STEAL AN EGG - INTEGRATED LOADER 🔥
-- ===================================================================
if not game:IsLoaded() then
    game.Loaded:Wait()
end

-- Kiểm tra Executor an toàn
if identifyexecutor then
    local execName = tostring(identifyexecutor()):lower()
    if execName:find("solara") or execName:find("xeno") then
        game:GetService("Players").LocalPlayer:Kick("EXECUTOR NOT SUPPORTED [SOLARA/XENO]")
        return
    end
end
...
print("[Yeager Hub] Đang khởi chạy hệ thống Steal An Egg (Integrated)...")
...
-- Giao diện chính (Tabs Interface)
...
StatusLabel.Text = "📊 Trạng thái: Hệ thống sẵn sàng"
...
createToggle("Main", "🚀 Auto Steal (Trộm trứng tự động)", function(state)
    getgenv().Config.AutoSteal = state
end)

createToggle("Main", "🥚 Auto Hatch (Ấp trứng tự động)", function(state)
    getgenv().Config.AutoHatch = state
end)

createToggle("Main", "📍 Auto Place (Đặt vị trí tự động)", function(state)
...
createToggle("ServerHop", "🌐 Auto Server Hop (Khi hết trứng)", function(state)
...
-- Luồng thực thi Auto Steal & Server Hop
task.spawn(function()
            ...
            pcall(function()
                ...
                if char and char:FindFirstChild("HumanoidRootPart") then
                    ...
                    for _, obj in pairs(Workspace:GetDescendants()) do
                        ...
                        if obj:IsA("ProximityPrompt") then
                            ...
                            if nameLower:match("egg") or parentName:match("egg") or obj.ActionText:lower():match("steal") then
                                ...
                                if partCFrame then
                                    found = true
                                    StatusLabel.Text = "🎯 Đang thực hiện Auto Steal..."
                    ...
                    if not found and getgenv().Config.AutoServerHop then
                        StatusLabel.Text = "🌐 Đang đổi server do hết trứng..."
                        task.wait(1)
                        TeleportService:Teleport(game.PlaceId, LocalPlayer)
                    end
                end
...
print("[Yeager Hub] Steal An Egg Ultimate Loaded Successfully!")
